...
{
  CG_LOG_INFO("Transcription process started for file: %s", path.c_str());
  std::filesystem::path infilepath(path);
  // Load audio and tokenizer.
  const capgen::AudioDecoder audio_decoder = capgen::AudioDecoder();
  at::Tensor spectrogram = audio_decoder.get_audio_spectrogram(infilepath.c_str());
  ...
  while (seek < spectrogram.size(-1)) {
    ...
    // We add 1 to segment index to potentially avoid using kv cache, cached during language prediction.
    capgen::greedy_decode_segment(segment_spec, task, language_id, segment_index + 1,  whisper, tokenizer, transcriptions);
    frames_transcribed += transcriptions[segment_index].end_time * 100;
    seek += (int)(transcriptions[segment_index].end_time * 100);
    segment_index += 1;
